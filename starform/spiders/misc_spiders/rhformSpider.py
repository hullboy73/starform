...
from starform.items import rhformItem
import os
import re
import unicodecsv
...
class rhformSpider(Spider):
	...
	def parse(self, response):
		...
		formxpath = response.selector.xpath("/html/body/div/div[2]/div[1]/div[4]/div/div[2]/table/tbody/tr")
		...
		for eachrow in formxpath:			
			try:
				...
				item['jockey']     = "".join(eachrow.xpath('td[9]/p/a/text()').extract()).strip()
				item['jkid']   	   = "".join(eachrow.xpath('td[9]/p/a/@href').extract()).replace("/jockey?jockeyId=1.","")
