...
from starform.items import jkformItem
import os
import re
import unicodecsv
...
def read_starturls(links_path):
	...
	with open(links_path, 'r') as infile:
		...
		for eachrow in csvlist[1:]:
			resultpage = 'http://form.horseracing.betfair.com{}'.format(eachrow[18].encode('utf-8'))
			resultpages.append(resultpage)
	return resultpages

class jkformSpider(Spider):
	name = "jkform"
	...
	def parse(self, response):
		...
		try:
			item['jockey'] = "".join(response.selector.xpath("/html/body/div/div[2]/div[1]/div[1]/div/div/div[1]/h2/text()").extract())
			item['jkid']      = response.url.replace("http://form.horseracing.betfair.com/jockey?jockeyId=1.","")
			# print jockey
			# print jkid
		except:
			pass
		for eachrow in formxpath:			
			try:
				...
				item['trainer']    = "".join(eachrow.xpath('td[9]/p[2]/a/text()').extract()).strip()
				item['trid']       = "".join(eachrow.xpath('td[9]/p[2]/a/@href').extract()).replace("/trainer?trainerId=","")
